...
class Generator(nn.Module):
    def __init__(self, img_size=256, patch_size=16, in_chans=3, num_classes=10, window_size=16, latent_dim=512, embed_dim=1024, depth=[5, 4, 4, 4, 4, 4],
        ...
        if self.l2_size == 0:
            ...
            self.l1 = nn.Linear(self.latent_dim, (self.bottom_width ** 2) * self.l2_size//16)
            self.l2 = nn.Sequential(
                        nn.Linear(self.l2_size//16, self.l2_size),
                        nn.Linear(self.l2_size, self.embed_dim)
                      )
        else:
            self.l1 = nn.Linear(self.latent_dim, (self.bottom_width ** 2) * self.l2_size)
            self.l2 = nn.Linear(self.l2_size, self.embed_dim)
        self.embedding_transform = nn.Linear(self.latent_dim, (self.bottom_width ** 2) * self.embed_dim)
...
#                         )
    ...
    def forward(self, z):
        ...
        # input noise
        x = x + self.to_device(self.pos_embed[0], x.get_device())
        B = x.size(0)
        H, W = self.bottom_width, self.bottom_width

        # embedding
        embedding = self.embedding_transform(z).view(-1, self.bottom_width ** 2, self.embed_dim)
        embedding = embedding + self.to_device(self.embed_pos, embedding.get_device())
...
#         x, _ = self.blocks_1(x, embedding)
        ...
        x = x + self.to_device(self.pos_embed[1], x.get_device())
        B, _, C = x.size()
        x, _ = self.blocks_2(x, embedding)
        
        x, H, W = bicubic_upsample(x, H, W)
        x = x + self.to_device(self.pos_embed[2], x.get_device())
        B, _, C = x.size()
...
#         x, H, W = updown(x, H, W)
        ...
        x = x + self.to_device(self.pos_embed[3], x.get_device())
        B, _, C = x.size()
#         x = x.view(B, H, W, C)
#         x = window_partition(x, self.window_size)
#         x = x.view(-1, self.window_size*self.window_size, C)
...
#         x = window_reverse(x, self.window_size, H, W).view(B,H*W,C)
        ...
        x, H, W = pixel_upsample(x, H, W)
        x = x + self.to_device(self.pos_embed[4], x.get_device())
        B, _, C = x.size()
#         x = x.view(B, H, W, C)
#         x = window_partition(x, self.window_size)
#         x = x.view(-1, self.window_size*self.window_size, C)
...
#         x = window_reverse(x, self.window_size, H, W).view(B,H*W,C)
        ...
        x, H, W = pixel_upsample(x, H, W)
        x = x + self.to_device(self.pos_embed[5], x.get_device())
        B, _, C = x.size()
#         x = x.view(B, H, W, C)
#         x = window_partition(x, self.window_size)
#         x = x.view(-1, self.window_size*self.window_size, C)
