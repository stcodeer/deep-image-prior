...
class Generator(nn.Module):
    ...
    def forward(self, z):
        ...
        x, H, W = bicubic_upsample(x, H, W)# x2
        x = x + self.pos_embed[1]
        B, _, C = x.size()
        x = self.blocks_2(x)
        
        x, H, W = bicubic_upsample(x, H, W)# x4
        x = x + self.pos_embed[2]
        B, _, C = x.size()
        x = self.blocks_3(x)
        
        x, H, W = pixel_upsample(x, H, W)# x8
        ...
        x, H, W = pixel_upsample(x, H, W)# x16
        ...
        x, H, W = pixel_upsample(x, H, W)# x32
