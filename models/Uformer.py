...
class UNet(nn.Module):
    def __init__(self, in_chans=3, out_chans=3, block=ConvBlock,dim=32):
        super(UNet, self).__init__()

        self.dim = dim
        self.ConvBlock1 = ConvBlock(in_chans, dim, strides=1)
        ...
        self.conv10 = nn.Conv2d(dim, out_chans, kernel_size=3, stride=1, padding=1)
...
def get_arch(arch, img_size, embed_dim=32, in_chans=3, out_chans=3):
    if arch == 'UNet':
        model_restoration = UNet(dim=embed_dim, in_chans=in_chans, out_chans=out_chans)
    elif arch == 'Uformer':
        model_restoration = Uformer(img_size=img_size,embed_dim=embed_dim,win_size=8,token_projection='linear',token_mlp='leff',modulator=True, dd_in=in_chans, in_chans=out_chans)
    elif arch == 'Uformer_T':
        model_restoration = Uformer(img_size=img_size,embed_dim=16,win_size=8,token_projection='linear',token_mlp='leff',modulator=True, dd_in=in_chans, in_chans=out_chans)
    elif arch == 'Uformer_S':
        model_restoration = Uformer(img_size=img_size,embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff',modulator=True, dd_in=in_chans, in_chans=out_chans)
    elif arch == 'Uformer_S_noshift':
        model_restoration = Uformer(img_size=img_size,embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff',modulator=True, dd_in=in_chans, in_chans=out_chans,
            shift_flag=False)
    elif arch == 'Uformer_B_fastleff':
        model_restoration = Uformer(img_size=img_size,embed_dim=32,win_size=8,token_projection='linear',token_mlp='fastleff', dd_in=in_chans, in_chans=out_chans,
            depths=[1, 2, 8, 8, 2, 8, 8, 2, 1],modulator=True)  
    elif arch == 'Uformer_B':
        model_restoration = Uformer(img_size=img_size,embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff', dd_in=in_chans, in_chans=out_chans,
            depths=[1, 2, 8, 8, 2, 8, 8, 2, 1],modulator=True)  
    else:
