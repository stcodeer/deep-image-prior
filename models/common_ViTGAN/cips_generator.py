...
class Generator(nn.Module):
    def __init__(self, size,
        ...
        if small32:
            ...
            self.channels = {
                ...
                2: 512,
                3: 512,
                4: int(256 * channel_multiplier),
                5: int(128 * channel_multiplier),
                6: int(64 * channel_multiplier),
                7: int(32 * channel_multiplier),
                8: int(16 * channel_multiplier),
            }
        ...
        self.input = ConstantInput(self.channels[0], size=size)

        self.coords = convert_to_coord_format(1, size, size, integer_values=False, device='cpu')#self.device)


        self.conv1 = StyleLayer(
            2 * style_dim, self.channels[0], 1, style_dim, blur_kernel=blur_kernel
        )
    ...
    def forward(self, input,
        ...
        if self.training and w == h == self.size:
            ...
            emb = F.grid_sample(
                self.input.input.expand(bs, -1, -1, -1),
