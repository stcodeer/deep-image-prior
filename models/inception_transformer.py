...
class PatchDeEmbed(nn.Module):
    def __init__(self, in_chans, embed_dim):
        ...
        # self.proj = nn.ConvTranspose2d(in_chans, embed_dim, kernel_size=3, stride=2, output_padding=1, padding=1)
        # self.norm = nn.BatchNorm2d(embed_dim)
    ...
    def forward(self, x):
        ...
        # x = rearrange(x, 'b h w (p1 p2 c)-> b (h p1) (w p2) c', p1=2, p2=2, c=self.embed_dim)
...
class FinalPatchDeEmbed(nn.Module):
    def __init__(self, in_chans, embed_dim):
        ...
        # self.embed_dim = embed_dim
        # self.proj1 = nn.ConvTranspose2d(in_chans, embed_dim * 2, kernel_size=3, stride=2, output_padding=1, padding=1)
        # self.norm1 = nn.BatchNorm2d(embed_dim * 2)
        # self.gelu1 = nn.GELU()
        # self.proj2 = nn.ConvTranspose2d(embed_dim * 2, embed_dim, kernel_size=3, stride=2, output_padding=1, padding=1)
        # self.norm2 = nn.BatchNorm2d(embed_dim)
    ...
    def forward(self, x):
        ...
        # x = rearrange(x, 'b h w (p1 p2 c)-> b (h p1) (w p2) c', p1=4, p2=4, c=self.embed_dim)
...
class InceptionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, out_chans=3, embed_dims=None, depths=None,
                 ...
                 drop_rate=0.1, attn_drop_rate=0.1, drop_path_rate=0.1, embed_layer=PatchEmbed, de_embed_layer=PatchDeEmbed, norm_layer=None,
        ...
        self.patch_embed8 = FinalPatchDeEmbed(in_chans=embed_dims[0], embed_dim=out_chans)
