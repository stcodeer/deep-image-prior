# Swin 的代码目前有问题，应该把 Encoder + 大全连接层 换成 Encoder + Decoder
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.zoom_radio = 4 * (2 ** (self.num_layers - 1))
        self.lines = img_size[0] * img_size[1] // self.zoom_radio // self.zoom_radio
        # self.head = []
        # for index in range(self.lines):
        #     self.head.append(nn.Linear(self.num_features, in_chans * self.zoom_radio * self.zoom_radio))
    ...
    def forward(self, x):
        ...
        # y = torch.zeros(1, self.lines, self.in_chans * self.zoom_radio * self.zoom_radio)
        # for index in range(self.lines):
        #     y[0][index] = self.head[index](x[0][index])
...
# model = SwinTransformer(img_size=(h, w))
# --------------------------------------------------------
# Swin Transformer
# Copyright (c) 2021 Microsoft
# Licensed under The MIT License [see LICENSE for details]
